...
public class BleConnector {
    ...
    private BleNotifyCallback.BleImmedimmediateCallback bleImmedimmediateCallback;
    ...
    BleConnector(BleBluetooth bleBluetooth) {
        ...
        this.bleImmedimmediateCallback = new BleBaseCallback.BleImmedimmediateCallback() {
            @Override
            public void bleCallBackImmediate(Object key, Bundle bundle, Object callback) {
                Integer keyInt = (Integer) key;
                switch (keyInt) {
                    case BleMsg.MSG_CHA_NOTIFY_DATA_CHANGE: {
                        BleNotifyCallback notifyCallback = (BleNotifyCallback) callback;
                        byte[] value = bundle.getByteArray(BleMsg.KEY_NOTIFY_BUNDLE_VALUE);
                        if (notifyCallback != null) {
                            notifyCallback.onCharacteristicChanged(value);
                        }
    ...
    private void handleCharacteristicNotifyCallback(BleNotifyCallback bleNotifyCallback,
        ...
        if (bleNotifyCallback != null) {
            ...
            if (bleNotifyCallback.isImmediate()) {
                // 添加立即执行callback的方法，防止丢包
                bleNotifyCallback.setBleImmedimmediateCallback(bleImmedimmediateCallback);
            }
